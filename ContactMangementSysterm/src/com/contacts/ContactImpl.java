...
public class ContactImpl implements DataAccess<ContactInfo> {
	...
	public int add(ContactInfo t) {
	   ...
	   String sql="select count(*) from contact_details";
	   ...
	   String numbercheck="select contactName from contact_number where contactNumber=?";
		...
		//finding number of contact available in contact_detail table
		try(PreparedStatement pstmt = con.prepareStatement(sql)) {
			...
			if (row!=0) {
				//checking if given name already in contact list
				//if name is already present then user can either modify name or can add number for existing user
				try(PreparedStatement checkstmt = con.prepareStatement(namecheck)) 
				{
					...
					if(check.next())
					{
						...
						System.out.print("\n"+t.getName().toUpperCase()+" Already Exist \n 1) Add Contact For existing Name\n 2) Modify Name\nSelect Option := ");
						option = sc.nextInt();
						//if want to add number for existing user
						if(option==1)
						{
							...
							try( PreparedStatement nstmt = con.prepareStatement(numberSql)) {
								...
								rowsAdded2 =  nstmt.executeUpdate(); 
						...
						//if you want to provide new name for current contact because entered name already present in contact list
						if(option==2)
						{
							System.out.print("\nEnter New Name: ");
			...
			if((row==0)||(checkNamecond))
			{
				
				contactId= t.getName().charAt(0)+t.getAddress().charAt(0)+t.getMobileNumber();
				...
				try(PreparedStatement detailStmt = con.prepareStatement(detailsql) ) {
					...
					try( PreparedStatement numberStmt = con.prepareStatement(numbersql)) {
						...
						rowsAdded1 =  detailStmt.executeUpdate(); 
